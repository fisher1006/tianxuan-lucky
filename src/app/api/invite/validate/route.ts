  ...
  try {
    ...
    if (!code || typeof code !== 'string') {
      return NextResponse.json(
        { valid: false, status: 'invalid', error: '请输入邀请码' },
        { status: 400 }
      );
    }

    const upstream = await fetch(`${getInviteApiBaseUrl()}/api/invite/validate`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { valid: false, status: 'invalid', error: message },
      { status: 500 }
    );
  }
}
